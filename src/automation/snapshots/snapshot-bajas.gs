 ...
 * Read pending TERMINATION (BAJAS) requests and snapshot to queue
 * 
 * Flow:
 * 1. Read "Ordered Responses" sheet
 * 2. Filter rows where Type = "BAJA" AND Status = "" (pending)
 * 3. Deduplicate by timestamp
 * 4. Assign sequential IDs
 * 5. Write to "Gestion BAJAS - WORKDAY" queue sheet
 */

/**
 * Snapshot pending BAJA (termination) requests from ordered responses
 ...
 */
function snapshotBajasPendientesToQueue() {
  try {
    ...
    if (!sheetRO || !sheetQueue) {
      throw new Error('Missing required sheets: Ordered Responses or BAJAS queue');
    }
    ...
    if (lastRow <= HEADER_ROW_RO) {
      Logger.log('snapshotBajasPendientesToQueue: No data rows');
    ...
    // Filter: Type = "BAJA" AND Status = "" (pending)
    const pendingBajas = allRows.filter(row => {
      const tipoGestion = toTrimmedString_(row[colsRO.TIPO_GESTION - 1]);
      const status = toTrimmedString_(row[colsRO.STATUS - 1]);
      return tipoGestion === 'BAJA' && status === '';
    });
    
    if (pendingBajas.length === 0) {
      Logger.log('snapshotBajasPendientesToQueue: No pending BAJA requests');
      return { success: true, rowsProcessed: 0, message: 'No pending BAJA requests' };
    }
    
    // Deduplicate by timestamp
    const seen = new Set();
    const deduplicated = [];
    pendingBajas.forEach(row => {
    ...
    Logger.log('snapshotBajasPendientesToQueue: Processed ' + deduplicated.length + ' BAJA requests');
    return {
      success: true,
      rowsProcessed: deduplicated.length,
      message: 'Snapshot completed: ' + deduplicated.length + ' BAJA requests copied to queue'
    };
    
  } catch (e) {
    Logger.error('snapshotBajasPendientesToQueue ERROR: ' + e.message);
